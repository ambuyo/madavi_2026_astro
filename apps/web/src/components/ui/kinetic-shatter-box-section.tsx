...
export const BreakableCard = ({ title, subtitle, description, isBullet, className, onBreak, pricing }: SectionCardProps) => {
    const {
        isDragging,
        position,
        crackLevel,
        ...
        <div className={cn("relative w-full h-full", className)}>
            ...
            >
                ...
                <div className="mt-8 border-t-2 border-black pt-2 flex justify-between text-xs font-mono uppercase">
                    <span>{subtitle ? title : `Fig. ${figNum.toString().padStart(2, '0')}`}</span>
                    <span className={cn(crackLevel > 0.7 && "text-red-600 font-bold")}>
                        {pricing ? pricing : (crackLevel > 0 ? `${Math.round(crackLevel * 100)}% DMG` : 'INTACT')}
                    </span>
                </div>
